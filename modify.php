        ...
        $prod_escaped = mysqli_real_escape_string($con, $prod);
            ...
            if ($_POST['action'] === 'add') {
                ...
                $req = "UPDATE Produits 
                        ...
                        WHERE Nom_Produit = '$prod_escaped' AND Quantite >= $quantite";
                mysqli_query($con, $req);
                $rq = "SELECT Quantite FROM Produits WHERE Nom_Produit = '$prod_escaped'";
                $tab = mysqli_query($con, $rq);
                if($ligne = mysqli_fetch_assoc($tab)){
                    if($ligne['Quantite'] == 0){
                        $req = "DELETE FROM Produits WHERE Nom_Produit = '$prod_escaped'";
                        mysqli_query($con, $req);
                    }
                }
            }
